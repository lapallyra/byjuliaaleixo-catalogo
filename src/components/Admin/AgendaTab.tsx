    ...
    <div className="space-y-6" style={{ fontFamily: 'Tahoma, Verdana, Segoe UI, sans-serif' }}>
      ...
      <div className="relative w-full max-w-xl mx-auto mb-10">
        ...
        <input 
          ...
          className="w-full pl-12 pr-4 py-4 rounded-3xl bg-[#140b0e] border border-lilac/30 text-[10px] uppercase font-bold tracking-widest outline-none focus:border-lilac transition-all shadow-sm text-rose-50"
          value={searchTerm}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl font-black text-rose-50 uppercase tracking-tighter">
              {safeFormat(currentMonth, 'MMMM yyyy', { locale: ptBR })}
            </h2>
            <div className="flex gap-2">
              <button 
                onClick={() => setCurrentMonth(addMonths(currentMonth, -1))} 
                className="p-3 rounded-xl bg-[#140b0e] border border-lilac/20 hover:bg-lilac text-rose-50 hover:text-white transition-all shadow-sm group"
              >
          ...
          <div className="grid grid-cols-7 gap-px bg-lilac/20 rounded-3xl overflow-hidden border border-lilac/20 shadow-xl">
            {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(d => (
              <div key={d} className="bg-[#140b0e] p-4 text-center text-[10px] font-black uppercase tracking-widest text-rose-50">{d}</div>
            ))}
            {(() => {
              ...
              days.forEach((day) => {
                ...
                cells.push(
                  <div key={day.toISOString()} className="min-h-[160px] bg-[#140b0e] p-1 hover:bg-lilac/5 transition-all group flex flex-col gap-0.5 border border-transparent">
                    <span className={`text-[10px] font-black transition-colors mb-1 ml-1 ${isSameDay(day, new Date()) ? 'text-lilac' : 'text-rose-300 group-hover:text-rose-50'}`}>
                      {safeFormat(day, 'd')}
                    </span>
                    <div className="flex-1 space-y-0.5 overflow-hidden">
                      {dayOrders.map((order, oIdx) => {
                        ...
                        return (
                          ...
                          >
                             {isStart && <div className="w-1 h-1 rounded-full bg-[#140b0e] shrink-0" />}
                             <span className="text-[7px] font-black uppercase text-white truncate leading-none">
                               {showLabel ? order.customerName : '\u00A0'}
                             </span>
                             {isEnd && <div className="ml-auto w-1 h-1 rounded-full bg-[#140b0e]/50 shrink-0" />}
                          </div>
                        )
                      })}
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
            {filteredOrders.filter(o => o.status === 'production' || o.status === 'pending').slice(0, 6).map(order => {
              ...
              <div 
                key={order.id} 
                onClick={() => onSelectOrder?.(order.id)}
                className="p-5 rounded-3xl bg-[#140b0e] border border-lilac/10 flex items-center gap-4 group hover:border-lilac/40 transition-all shadow-sm cursor-pointer"
              >
                ...
                <div className="flex-1 overflow-hidden">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className="text-[10px] font-black text-rose-50 uppercase truncate">{order.customerName}</span>
                    <span className={`text-[8px] font-mono font-black text-${colorBase === 'lilac' ? 'lilac' : colorBase}`}>#{order.code}</span>
                  </div>
                  <p className="text-[8px] text-rose-300 font-bold uppercase tracking-widest">Entrega: {safeFormatISO(order.deliveryDate || '', 'dd/MM/yyyy')}</p>
                </div>
              </div>
            );})}
          </div>
