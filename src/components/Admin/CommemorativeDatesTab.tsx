  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-4xl font-display font-black text-rose-50 tracking-tight">Datas Comemorativas</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium">Sua bússola de marketing e planejamento estratégico.</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex bg-[#2b141e] p-1 rounded-2xl border border-rose-900/50/50">
            <button 
              onClick={() => setViewMode('list')}
              className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'list' ? 'bg-[#140b0e] text-rose-50 shadow-sm' : 'text-rose-300 hover:text-slate-600'}`}
            >
              Lista
            </button>
            <button 
              onClick={() => setViewMode('calendar')}
              className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'calendar' ? 'bg-[#140b0e] text-rose-50 shadow-sm' : 'text-rose-300 hover:text-slate-600'}`}
            >
              Calendário
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-2xl shadow-indigo-500/20">
           ...
           <div className="relative z-10">
             <div className="flex items-center gap-3 mb-8">
                <div className="w-12 h-12 bg-[#140b0e]/20 rounded-2xl flex items-center justify-center backdrop-blur-md">
             ...
             {todayDates.length > 0 ? (
               <div className="space-y-6">
                 ...
                 <div className="flex gap-4 pt-4">
                   <button className="bg-[#140b0e] text-indigo-600 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#1f0e16] transition-all flex items-center gap-2">
        ...
        <div className="bg-[#140b0e] rounded-[3rem] p-10 border border-rose-900/30 shadow-xl shadow-slate-200/50">
           <h4 className="text-xs font-black uppercase tracking-widest text-rose-300 mb-8 flex items-center gap-2">
           ...
           <div className="space-y-6">
             {upcomingDates.length > 0 ? upcomingDates.map((d, i) => (
               <div key={d.id} className="flex items-center gap-4 group">
                 <div className="w-10 h-10 rounded-xl bg-[#1f0e16] flex flex-col items-center justify-center text-rose-300 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-all">
                 ...
                 <div>
                   ...
                   <p className="text-[9px] text-rose-300 font-bold uppercase tracking-tight">{categories.find(c => c.id === d.category)?.label}</p>
                 </div>
               </div>
             )) : (
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="flex-1 relative group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-rose-300 group-focus-within:text-rose-50 transition-colors" size={20} />
          <input 
            type="text"
            placeholder="Buscar por nome, descrição ou hashtag..."
            className="w-full h-14 bg-[#140b0e] border border-rose-900/50 rounded-3xl pl-12 pr-6 text-sm font-medium focus:ring-4 focus:ring-slate-100 focus:border-slate-400 transition-all outline-none shadow-sm"
            value={searchTerm}
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2">
          <button 
           ...
           className={`px-6 py-3 rounded-full text-xs font-black uppercase tracking-widest whitespace-nowrap transition-all border ${selectedCategory === 'all' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-[#140b0e] border-rose-900/30 text-slate-500 hover:bg-[#1f0e16]'}`}
          >
            Todos
          </button>
          {categories.map(cat => (
            <button 
              ...
              className={`px-6 py-3 rounded-full text-xs font-black uppercase tracking-widest whitespace-nowrap transition-all border flex items-center gap-2 ${selectedCategory === cat.id ? 'bg-[#140b0e] border-slate-900 text-rose-50 shadow-lg' : 'bg-[#140b0e] border-rose-900/30 text-slate-500 hover:bg-[#1f0e16]'}`}
            >
        ...
        {viewMode === 'list' ? (
          ...
          >
            {filteredDates.map((date) => {
              ...
              return (
                <div key={date.id} className="bg-[#140b0e] rounded-[2.5rem] border border-rose-900/30 p-8 hover:shadow-2xl hover:shadow-slate-200/50 transition-all group relative overflow-hidden">
                  {!date.active && (
                    <div className="absolute inset-0 bg-[#140b0e]/60 backdrop-blur-[2px] z-10 flex items-center justify-center">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                          ...
                          <p className="text-[10px] text-rose-300 font-bold uppercase tracking-widest">{catInfo?.label}</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                       <button onClick={() => { setEditingDate(date); setIsModalOpen(true); }} className="p-2 hover:bg-[#1f0e16] rounded-xl text-rose-300 hover:text-rose-50 transition-all"><Edit2 size={16} /></button>
                       <button onClick={() => handleDelete(date.id)} className="p-2 hover:bg-[#1f0e16] rounded-xl text-rose-300 hover:text-rose-500 transition-all"><Trash2 size={16} /></button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {date.hashtags.map((tag, i) => (
                      <span key={`tag-${tag}-${i}`} className="text-[9px] font-bold text-rose-300 lowercase">#{tag}</span>
                    ))}
                  </div>
          ...
          <motion.div 
            ...
            animate={{ opacity:1, y:0 }}
            exit={{ opacity:0, y:-20 }}
            className="bg-[#140b0e] rounded-[3rem] border border-rose-900/30 shadow-xl overflow-hidden"
          >
            <div className="p-10 border-b border-slate-50 flex items-center justify-between">
               <h3 className="text-xl font-display font-black text-rose-50 uppercase tracking-widest">{format(currentMonth, 'MMMM yyyy', { locale: ptBR })}</h3>
               <div className="flex gap-2">
                 <button onClick={() => setCurrentMonth(addDays(startOfMonth(currentMonth), -1))} className="p-3 bg-[#1f0e16] hover:bg-[#2b141e] rounded-2xl transition-all"><ChevronLeft size={20} /></button>
                 <button onClick={() => setCurrentMonth(addDays(endOfMonth(currentMonth), 1))} className="p-3 bg-[#1f0e16] hover:bg-[#2b141e] rounded-2xl transition-all"><ChevronRight size={20} /></button>
               </div>
            </div>
            
            <div className="grid grid-cols-7 bg-[#1f0e16]/50">
               {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(d => (
                 <div key={d} className="p-4 text-center text-[10px] font-black uppercase tracking-[0.3em] text-rose-300 border-r border-rose-900/30 last:border-0">{d}</div>
               ))}
            </div>
            ...
            <div className="grid grid-cols-7 min-h-[600px]">
               {(() => {
                 ...
                 for (let i = 0; i < emptyDays; i++) {
                   cells.push(<div key={`empty-${i}`} className="p-4 border-r border-b border-slate-50 bg-[#1f0e16]/20" />);
                 }
                 ...
                 for (let d = 1; d <= totalDays; d++) {
                   ...
                   cells.push(
                     <div key={d} className={`p-4 border-r border-b border-slate-50 relative group hover:bg-[#1f0e16]/30 transition-colors min-h-[120px] ${isToday(cellDate) ? 'bg-indigo-50/30' : ''}`}>
                        <span className={`text-xs font-black transition-colors ${isToday(cellDate) ? 'text-indigo-600' : 'text-slate-300 group-hover:text-rose-50'}`}>{d}</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-12">
          ...
          <motion.div 
            layoutId="modal-date"
            className="bg-[#140b0e] w-full max-w-2xl rounded-[3rem] shadow-2xl relative z-10 overflow-hidden"
          >
            <div className="p-10 overflow-y-auto max-h-[85vh] no-scrollbar">
               <div className="flex justify-between items-center mb-10">
                 <div>
                   <h3 className="text-2xl font-display font-black text-rose-50">{editingDate?.id ? 'Editar Data' : 'Nova Data Comemorativa'}</h3>
                   <p className="text-rose-300 text-xs font-medium uppercase tracking-widest mt-1">Configure os parâmetros da data estratégica.</p>
                 </div>
                 <button onClick={() => setIsModalOpen(false)} className="p-3 bg-[#2b141e] hover:bg-[#3e1b29] rounded-2xl transition-all"><X size={20} /></button>
               </div>

               <form onSubmit={handleSave} className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Nome da Data</label>
                       <input 
                         type="text"
                         required
                         value={editingDate?.name || ''}
                         onChange={e => setEditingDate({ ...editingDate, name: e.target.value })}
                         className="w-full h-14 bg-[#1f0e16] border border-rose-900/30 rounded-2xl px-6 text-sm font-bold focus:bg-[#140b0e] focus:ring-4 focus:ring-slate-100 transition-all outline-none"
                         placeholder="Ex: Dia do Designer"
                       />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Categoria</label>
                       <select 
                         value={editingDate?.category}
                         onChange={e => setEditingDate({ ...editingDate, category: e.target.value as CategoryId })}
                         className="w-full h-14 bg-[#1f0e16] border border-rose-900/30 rounded-2xl px-6 text-sm font-bold focus:bg-[#140b0e] outline-none"
                       >
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Descrição / Curiosidade</label>
                    <textarea 
                      value={editingDate?.description || ''}
                      onChange={e => setEditingDate({ ...editingDate, description: e.target.value })}
                      className="w-full min-h-[100px] bg-[#1f0e16] border border-rose-900/30 rounded-2xl p-6 text-sm font-medium focus:bg-[#140b0e] outline-none resize-none"
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Dia</label>
                       <input 
                         type="number"
                         min="1" max="31"
                         ...
                         className="w-full h-14 bg-[#1f0e16] border border-rose-900/30 rounded-2xl px-6 text-sm font-bold focus:bg-[#140b0e] outline-none disabled:opacity-30"
                       />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Mês</label>
                       <select 
                         disabled={!editingDate?.year_fixed}
                    ...
                    <div className="flex flex-col justify-center">
                       <label className="flex items-center gap-3 cursor-pointer group">
                         <div className={`w-12 h-6 rounded-full p-1 transition-all duration-300 ${editingDate?.year_fixed ? 'bg-indigo-600' : 'bg-[#3e1b29]'}`}>
                           <div className={`w-4 h-4 bg-[#140b0e] rounded-full shadow-sm transition-all duration-300 ${editingDate?.year_fixed ? 'ml-6' : 'ml-0'}`} />
                         </div>
                         <input 
                           type="checkbox" 
                           className="hidden" 
                         ...
                         />
                         <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 group-hover:text-rose-50">Data Fixa?</span>
                       </label>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-rose-300 ml-2">Frase de Marketing SUGERIDA</label>
                    <input 
                      type="text"
                  ...
                  <div className="flex justify-end gap-4 pt-6">
                     <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-rose-300 hover:text-rose-50 transition-all">Cancelar</button>
