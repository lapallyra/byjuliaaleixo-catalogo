...
export const FinanceTab: React.FC<FinanceTabProps> = ({ companyId, orders }) => {
  ...
  const TabNav = () => (
    <div className="flex flex-wrap gap-2 mb-10 sticky top-0 z-20 bg-[#140b0e]/80 backdrop-blur-md py-4 border-b border-rose-900/30">
      {[
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeSection === sec.id ? 'bg-lilac text-rose-50 shadow-lg shadow-lilac/20' : 'bg-[#140b0e] text-rose-300 hover:bg-[#2b141e]'}`}
        >
          <sec.icon size={14} />
          {sec.label}
        </button>
  ...
  const renderActiveSection = () => {
    switch (activeSection) {
        ...
        return (
          <section className="space-y-8 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { label: 'Faturamento Bruto', val: grossRevenue, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-950/30', border: 'border-emerald-100' },
                { label: 'Lucro Líquido Estimado', val: netProfit, icon: DollarSign, color: 'text-lilac', bg: 'bg-lilac/10', border: 'border-lilac/20' },
                { label: 'Custos Dinâmicos (Mês)', val: totalExpenses, icon: TrendingDown, color: 'text-rose-500', bg: 'bg-[#1f0e16]', border: 'border-[#3e1b29]' },
                { label: 'Cancelamentos', val: orders.filter(o => o.status === 'cancelled' && isSameMonth(o.createdAt?.toDate ? o.createdAt.toDate() : new Date(o.createdAt), filterMonth)).length, isCount: true, icon: XCircle, color: 'text-gray-500', bg: 'bg-[#2b141e]', border: 'border-rose-900/50' },
              ].map((card, idx) => (
                <div key={card.label} className={`p-8 rounded-[2rem] bg-[#140b0e] border ${card.border} shadow-lg shadow-black/5 relative overflow-hidden group hover:scale-[1.02] transition-transform`}>
                  ...
                  <div className="flex items-center gap-4 relative z-10">
                     ...
                     <span className="text-[9px] font-black uppercase text-rose-300 tracking-widest">{card.label}</span>
                  </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-gradient-to-br from-slate-900 to-black text-white shadow-2xl relative overflow-hidden">
                 ...
                 <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                       <div className="w-14 h-14 rounded-2xl bg-[#140b0e]/10 flex items-center justify-center border border-white/20"><Target size={24} className="text-lilac" /></div>
                       <div>
                    ...
                    <div className="flex-1 w-full flex items-center justify-end gap-6">
                       <div className="w-full max-w-md h-4 rounded-full bg-[#140b0e]/5 border border-white/10 overflow-hidden relative shadow-inner">
                         <div 
                           className="h-full bg-gradient-to-r from-lilac to-pink-500 relative"
                           style={{ width: `${goalProgress}%` }}
                         >
                            <div className="absolute inset-0 bg-[#140b0e]/20 animate-pulse" />
                         </div>
                       </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
              {/* Entradas */}
              <div className="bg-[#140b0e] rounded-[2rem] border border-emerald-100 shadow-xl shadow-emerald-50/50 p-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-[10px] font-black bg-emerald-950/30 text-emerald-600 px-3 py-1 rounded-full">{currentMonthOrders.length} Pedidos</span>
                </div>
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
                  {currentMonthOrders.length > 0 ? currentMonthOrders.slice(0, 10).map((o, idx) => (
                    <div key={`order-entry-${o.id}-${idx}`} className="flex justify-between items-center p-4 bg-emerald-950/30/30 rounded-xl border border-emerald-100/50">
                      <div>
                    ...
                    <p className="text-center text-[10px] uppercase font-black tracking-widest text-rose-300 py-10">Nenhuma venda esse mês.</p>
                  )}
                </div>
              </div>

               {/* Saídas e Custos Base */}
               <div className="bg-[#140b0e] rounded-[2rem] border border-[#3e1b29] shadow-xl shadow-rose-50/50 p-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-[10px] font-black bg-[#1f0e16] text-rose-600 px-3 py-1 rounded-full">Baseado nas configs</span>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-4 bg-[#1f0e16]/30 rounded-xl border border-[#3e1b29]/50">
                    <div>
                  ...
                  <div className="flex justify-between items-center p-4 bg-amber-950/30/30 rounded-xl border border-amber-100/50">
                    <div>
            ...
            {/* History Section */}
            <div className="bg-[#140b0e] rounded-[2rem] border border-rose-900/30 shadow-xl p-8 mt-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
                  {history.map((h) => (
                    <div key={h.id} className="p-4 bg-[#1f0e16] rounded-xl border border-rose-900/30 text-center">
                      <p className="text-[8px] font-bold text-rose-300 uppercase tracking-widest">{h.month}</p>
      ...
      case 'analise':
        return (
          <section className="space-y-8 animate-in fade-in duration-300">
             <div className="p-10 rounded-[3rem] bg-[#140b0e] border border-rose-900/30 shadow-2xl flex flex-col items-center justify-center text-center min-h-[400px]">
                ...
                <p className="text-[10px] font-black text-rose-300 uppercase tracking-[0.3em] max-w-sm leading-relaxed">
  ...
  return (
    <div className="space-y-12 pb-32 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-3xl font-black text-rose-50 tracking-tighter uppercase italic">Hub Financeiro Premium</h2>
          <p className="text-rose-300 text-[10px] font-black uppercase tracking-[0.3em] mt-2">Visibilidade total da sua lucratividade</p>
        </div>
        <div className="flex items-center gap-4 bg-[#140b0e] p-2 rounded-2xl border border-rose-900/30 shadow-sm">
          <button className="p-3 bg-[#140b0e] hover:bg-[#2b141e] rounded-xl transition-all text-rose-300 hover:text-rose-50 hover:shadow-md"><Download size={18} /></button>
          <div className="h-6 w-px bg-[#3e1b29]" />
          <div className="flex items-center gap-3 px-4">
            <Calendar size={18} className="text-rose-50" />
            <select 
              value={format(filterMonth, 'yyyy-MM')}
              onChange={(e) => setFilterMonth(new Date(e.target.value + '-01T00:00:00'))}
              className="bg-transparent text-[10px] font-black uppercase tracking-widest outline-none cursor-pointer text-rose-50"
            >
              {[0,1,2,3,4,5,6].map(i => {
